...
__global__ void kernel_transitions(Event* d_all_events, int n_transitions, int* d_transitions, int n_tasks)
{
	...
	if (i < n_transitions)
	{
		...
		{
			int task0 = d_all_events[i].task;
			int task1 = d_all_events[i+1].task;
			d_transitions[i] = n_tasks*task0 + task1 + 1;
		}
	}
}
...
int* flow3()
{
	...
	try
	{
		...
		for(int r=0; r<n_runs; r++)
		{
			...
			kernel_transitions<<<blocks1, threads1>>>(d_all_events, n_transitions, d_transitions, n_tasks);
